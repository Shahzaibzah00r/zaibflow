...
func TestProjectDirMatchesClaudeLayout(t *testing.T) {
	...
	root := "/tmp/projects"
	got := ProjectDir(root, "/Users/max/Downloads/zaibflow")
	want := filepath.Join(root, "-Users-max-Downloads-zaibflow")
	if got != want {
...
func TestLatestInProjectReturnsMostRecentSession(t *testing.T) {
	...
	root := t.TempDir()
	projectDir := ProjectDir(root, "/Users/max/Downloads/zaibflow")
	...
	got, err := LatestInProject(root, "/Users/max/Downloads/zaibflow")
	if err != nil {
		t.Fatal(err)
	}
	if got.ID != "newer" {
