...
func TestSyncCreatesBinaryAndLaunchers(t *testing.T) {
	...
	root := t.TempDir()
	execPath := filepath.Join(root, "zaibflow-bin")
	...
	info, err := os.Lstat(filepath.Join(paths.BinDir, "zaibflow"))
	if err != nil {
		t.Fatal(err)
	}
	if info.Mode()&os.ModeSymlink != 0 {
		t.Fatal("zaibflow should be a regular file in normal mode, not a symlink")
	}
}
...
func TestSyncHomebrewSkipsCopyAndUsesAbsoluteSymlinks(t *testing.T) {
	...
	homebrewBin := filepath.Join(root, "homebrew", "bin", "zaibflow")
	...
	// zaibflow binary must NOT be copied into BinDir
	if _, err := os.Lstat(filepath.Join(paths.BinDir, "zaibflow")); err == nil {
		t.Fatal("zaibflow binary must not be copied into BinDir in Homebrew mode")
	}
...
func TestSyncHomebrewSkipsDynamicProviderSymlinks(t *testing.T) {
	...
	for _, name := range []string{"zf-or-kimi", "zf-myprovider"} {
		if _, err := os.Lstat(filepath.Join(paths.BinDir, name)); err == nil {
